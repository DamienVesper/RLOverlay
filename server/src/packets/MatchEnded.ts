...
export class MatchEndedPacket extends Packet<MatchEnded, GameState> {
    serialize = (raw: MatchEnded) => core.game.state;

    deserialize = (raw: MatchEnded) => {
        core.game.series[raw.data.winner_team_num]++;
        core.logger.debug(`Rocket League`, `Game ${core.game.series.reduce((a, b) => a + b)} ended. Winner: "${core.game.teams.get(raw.data.winner_team_num)?.name}"`);
