...
export class MatchEndedPacket extends Packet<MatchEnded, MatchEnded[`data`][`winner_team_num`]> {
    ...
    deserialize = (raw: MatchEnded) => {
        ...
        core.logger.debug(`Rocket League`, `Game ${core.game.series.reduce((a, b) => a + b)} ended. Winner: "${core.game.teams.get(raw.data.winner_team_num)?.name}". Series Score: ${core.game.series.join(`-`)}`);
